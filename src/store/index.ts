...
function getLocalCopilotAnswer(content: string): string | null {
  const normalized = content.toLowerCase();
  const asksModel =
    /\b(model|llm|gpt|openrouter)\b/.test(normalized) &&
    /\b(what|which|using|use|running|powered|configured)\b/.test(normalized);
  const asksFramework = /\b(langchain|langgraph|agent framework|framework)\b/.test(normalized);
  const asksPrompt =
    /\b(system prompt|prompt|instructions|guardrails?)\b/.test(normalized) &&
    /\b(where|show|what|which|find|using|use|see|read)\b/.test(normalized);
  const asksGrounding = /\b(hallucinat|grounded|grounding|guardrails?|trust|verify)\b/.test(normalized);

  if (asksPrompt) {
    return [
      'The system prompt is exposed as MCP resource `fix://prompts/trading-ops`.',
      'Source of truth: `src/fix_mcp/prompts/trading_ops.py`.',
      'Browser copy: `src/store/prompts.ts`.',
      'In this panel, use the prompt/document button in the header to inspect the active prompt.',
    ].join('\n');
  }

  if (asksModel || asksFramework || asksGrounding) {
    return COPILOT_IMPLEMENTATION_NOTE;
  }
...
export const useChat = create<ChatState>((set, get) => ({
  ...
  send: async (content: string) => {
    ...
    const localAnswer = getLocalCopilotAnswer(content);
    if (localAnswer) {
