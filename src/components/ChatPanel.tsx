...
import { COPILOT_IMPLEMENTATION_NOTE, COPILOT_MODEL_LABEL, SYSTEM_PROMPT } from '@/store/prompts';
import { Send, X, CheckCircle, AlertCircle, Loader2, AlertTriangle, Terminal, Radio, Key, Shield, Trash2, Copy, Check, FileText } from 'lucide-react';
...
export function ChatPanel() {
  ...
  const { mode, scenario, scenarioContext, controlMode, takeOverAsAgent, releaseToHuman, toggleCollab } = useSystem();
  const [input, setInput] = useState('');
  const [showKeyModal, setShowKeyModal] = useState(false);
  const [showPromptModal, setShowPromptModal] = useState(false);
  const [promptText, setPromptText] = useState(SYSTEM_PROMPT);
  const [promptSource, setPromptSource] = useState<'backend' | 'browser'>('browser');
  ...
  useEffect(() => {
    ...
    fetch('/api/prompts/trading-ops')
      .then((res) => {
        if (!res.ok) throw new Error('Prompt endpoint unavailable');
        return res.text();
      })
      .then((text) => {
        if (!cancelled && text.trim()) {
          setPromptText(text);
          setPromptSource('backend');
        }
      })
      .catch(() => {
        if (!cancelled) {
          setPromptText(SYSTEM_PROMPT);
          setPromptSource('browser');
        }
      });
    ...
    <div className="h-full flex flex-col bg-[var(--bg-base)]">
      ...
      <div className="h-12 flex items-center justify-between px-4 border-b border-[var(--border-dim)] bg-[var(--bg-surface)] shrink-0">
        <div className="flex items-center gap-2">
          <div className="w-6 h-6 rounded-md bg-[var(--bg-elevated)] border border-[var(--border-dim)] flex items-center justify-center">
              ...
              {modeButtons.map((modeButton) => (
                <div key={modeButton.key} className="group relative">
                  <button
                    ...
                    className={`text-[10px] font-bold px-2 py-1 rounded transition-colors ${modeButton.active ? modeButton.activeClass : 'text-[var(--text-muted)] hover:bg-[var(--bg-elevated)]'}`}
      ...
      {/* Prompt Modal */}
      {showPromptModal && (
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm" onClick={() => setShowPromptModal(false)}>
          <div className="w-[calc(100vw-2rem)] max-w-[760px] max-h-[86vh] bg-[var(--bg-elevated)] border border-[var(--border-bright)] rounded-xl shadow-2xl flex flex-col" onClick={e => e.stopPropagation()}>
            <div className="flex items-start justify-between gap-3 p-4 border-b border-[var(--border-dim)]">
              <div>
                <h3 className="text-[14px] font-bold text-[var(--text-primary)]">System Prompt</h3>
                <p className="mt-1 text-[11px] leading-relaxed text-[var(--text-muted)]">
                  Backend source: <span className="font-mono text-[var(--text-secondary)]">src/fix_mcp/prompts/trading_ops.py</span>
                  {' '}· MCP resource: <span className="font-mono text-[var(--text-secondary)]">fix://prompts/trading-ops</span>
                  {' '}· Showing: <span className="font-mono text-[var(--text-secondary)]">{promptSource}</span>
                </p>
              </div>
              <button onClick={() => setShowPromptModal(false)} className="p-1 rounded text-[var(--text-dim)] hover:text-[var(--text-primary)]"><X size={14} /></button>
            </div>
            <div className="p-4 border-b border-[var(--border-dim)] text-[12px] leading-relaxed text-[var(--text-secondary)]">
              {COPILOT_IMPLEMENTATION_NOTE}
            </div>
            <pre className="m-0 flex-1 overflow-auto p-4 text-[11px] leading-relaxed text-[var(--text-secondary)] whitespace-pre-wrap font-mono">
              {promptText}
            </pre>
          </div>
        </div>
          ...
          <div className="bg-[var(--bg-elevated)] border border-[var(--border-bright)] rounded-xl p-5 w-[340px] shadow-2xl" onClick={e => e.stopPropagation()}>
            ...
            <div className="flex gap-2">
              <button
                ...
                className="flex-1 py-2 rounded-lg bg-[var(--cyan)] text-white text-[13px] font-bold hover:bg-[var(--cyan)]/80 transition-colors"
              >
                Save
              </button>
              {openRouterKey && (
