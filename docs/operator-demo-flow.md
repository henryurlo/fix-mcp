...
If someone asks about the model, prompt, or agent framework, answer directly:

- System prompt: open Copilot and click the document icon, or show `src/fix_mcp/prompts/trading_ops.py`. MCP clients can read `fix://prompts/trading-ops`.
- Model: the current console sends chat requests to `openai/gpt-5.4` through OpenRouter.
- LangChain / LangGraph: neither is used in this repo. The agent boundary is MCP tools, resources, prompts, FastAPI, and the Next.js console.
- Hallucination control: facts are grounded in scenario JSON and engine state; actions are typed MCP calls; recovery is approval-gated; Trace records the evidence. Do not claim hallucinations are impossible.
